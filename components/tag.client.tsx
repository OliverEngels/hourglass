...
const Tag: React.FC<BadgeProps> = ({ color, text, onRemove }) => {
    return (
        <span className={`relative flex items-center justify-center rounded-md bg-${color}-100 px-2 py-1 text-xs font-medium text-${color}-700 ring-1 ring-inset ring-${color}-700/10 ml-1 mb-1`}>

            <span>{text}</span>

            {onRemove != null && <button className={`ml-1 fill-current relative rounded hover:bg-${color}-200`} onMouseUp={() => onRemove(text)}>
